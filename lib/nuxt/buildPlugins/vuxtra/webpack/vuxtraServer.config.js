...
export default function vuxtraServerConfig (name) {
    ...
    const config = {
        ...
        node: false,
        devtool: 'source-map',
        entry: glob.sync(this.options.srcDir + '/server/**/*.js').reduce((entries, entry) => Object.assign(entries, {[entry.replace(this.options.srcDir+'/server/', '').replace('.js', '')]: entry}), {}),
        output: {
            path: resolve(this.options.buildDir, 'server'),
            filename: '[name].js',
