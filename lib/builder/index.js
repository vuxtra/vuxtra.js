...
export default class VuxtraBuilder extends Tapable {
    ...
    async generateFiles (entryFiles) {
        ...
        _.forEach(entryFiles, (filePath, path) => {
            ...
            // lets load up file
            let entryFileObj = {}
            let remoteObj = require(join(this.options.buildDir, 'server', _.trimEnd(path,'/')+'.js'))
            console.log(remoteObj)
            _.forIn(remoteObj, (value,key) => {
                console.log(value, key)
                entryFileObj[key] = function () {
                    return this._internal(arguments)
                }
            })

            _.set(finalObj, _.replace(_.trim(path, '/'), /\//g, '.'), entryFileObj);
        })
        ...
        const content = template({
            customObj: un_eval(finalObj)
        })

        let clientPathJs = r(this.options.buildDir, 'client.js')
