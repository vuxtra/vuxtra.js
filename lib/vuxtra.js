...
let defaultOptions = {
    ...
    'socketserver': {
        ...
        workerController: __dirname + '/../lib/socketserver/sccWorker.js',
        brokerController: __dirname + '/../lib/socketserver/sccBroker.js',
...
for (var i in SOCKETCLUSTER_OPTIONS) {
    if (SOCKETCLUSTER_OPTIONS.hasOwnProperty(i)) {
        defaultOptions.socketserver[i] = SOCKETCLUSTER_OPTIONS[i]
    }
}
...
export default class Vuxtra {
    ...
    startDev (port = 3000, host = 'localhost') {
        // socketServer dev setup
        this.options.socketserver.environment = 'dev'
    ...
    startSocketCluster (port = 3000, host= 'localhost') {
        ...
        var masterControllerPath =  __dirname + '/../lib/socketserver/sccMaster.js'
        var workerControllerPath =  __dirname + '/../lib/socketserver/sccWorker.js';
        var brokerControllerPath =  __dirname + '/../lib/socketserver/sccBroker.js';
        ...
        var start =  () => {
            if (optionsControllerPath) {
                ...
                optionsController.run(this.options.socketserver, launch)
            } else {
                launch(this.options.socketserver)
            }
        }
